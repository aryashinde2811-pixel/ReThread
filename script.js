...
function initLoginPage() {
  ...
  document.querySelectorAll('[data-role]').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      document.querySelectorAll('[data-role]').forEach(b => b.classList.remove('ring-2', 'ring-green-400'));
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      setTimeout(() => {
        if (data.role === 'admin' || data.user.role === 'admin') window.location.href = 'admin.html';
        else window.location.href = 'index.html';
      }, 600);
    } catch (err) {
      console.error(err);
      showToast('Login failed to connect to server', 'error');
    }
  });
}
